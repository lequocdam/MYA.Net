...
) : IOrderService
{
    ...
    public async Task<Guid> CreateAsync(CreateRequest request, CancellationToken ct)
    {
        var currentUser = currentUserService.GetCurrent();

        var fromTask = addressRepository.GetByIdAsync(request.FromAddressId, ct);
        var toTask = addressRepository.GetByIdAsync(request.ToAddressId, ct);
